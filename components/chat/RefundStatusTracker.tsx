...
export default function RefundStatusTracker({
  ...
  const steps = [
    ...
    },
    {
      ...
      status: refundData.stages.approved
        ? "completed"
        : refundData.stages.received
          ? "current"
          : "pending",
      icon: "✅",
    ...
    },
    {
      ...
      status: refundData.stages.processing
        ? "current"
        : refundData.stages.approved
          ? "pending"
          : "pending",
      icon: "💳",
      date: undefined,
  ...
  return (
    <div className="mt-3 bg-gradient-to-br from-purple-50 to-pink-100 rounded-2xl p-5 border-2 border-purple-200 shadow-lg">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div
          className={`px-4 py-2 rounded-full text-sm font-bold text-white bg-gradient-to-r ${getStatusColor()}`}
        >
      ...
      {refundData.status !== "rejected" && refundData.status !== "credited" && (
        <div className="mt-3 bg-blue-50 border border-blue-200 rounded-lg p-3">
          ...
          <p className="text-lg font-bold text-blue-900 mt-1">
            {new Date(refundData.expectedCreditDate).toLocaleDateString(
              "en-IN",
              {
                day: "2-digit",
                month: "long",
      ...
      {refundData.status === "rejected" && (
        <div className="mt-3 bg-red-50 border-2 border-red-200 rounded-lg p-4">
          ...
          <p className="text-sm text-red-600 mt-2">
            This refund request was not eligible. Premium Tatkal tickets are
      ...
      {refundData.status !== "rejected" && (
        <div className="mt-3 pt-3 border-t border-purple-200">
          <p className="text-xs text-gray-600 text-center">
            💡 Refund will be processed within 7-10 working days from
            cancellation
          </p>
        </div>
      )}
