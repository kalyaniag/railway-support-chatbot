...
export default function ChatContainer({ isWidget = false }: ChatContainerProps) {
  ...
  const handleSendMessage = async (userMessage: string) => {
    ...
    setTimeout(() => {
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === botMsgId ? { ...msg, streaming: false, content: responseData.text, link: responseData.link, richContent: responseData.richContent } : msg
        )
      );
        ...
        <header className="bg-white shadow-md">
          <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-lg font-bold text-blue-900">INDIAN RAILWAYS</h1>
                <p className="text-xs text-gray-600">Safety | Security | Punctuality</p>
              </div>
            </div>
