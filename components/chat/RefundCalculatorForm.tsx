...
import { getToneConfig, isHighValueRefund } from '@/lib/chatbot/toneAdapter';
...
export default function RefundCalculatorForm({ onCalculate }: RefundCalculatorFormProps) {
  const [ticketType, setTicketType] = useState<'AC' | 'Sleeper' | 'Tatkal'>('AC');
    ...
    <div className="bg-white rounded-lg border border-orange-200 p-4 space-y-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">Calculate your estimated refund</p>
        </div>
      </div>
      ...
      {!showResult ? (
        <>
          ...
          <button
            ...
            className="w-full py-3 bg-gradient-to-r from-orange-600 to-orange-500 text-white rounded-lg font-semibold hover:from-orange-700 hover:to-orange-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
            Calculate Refund
          </button>
        </>
          ...
          <div className="space-y-3">
            <div className="p-3 bg-gradient-to-r from-orange-50 to-amber-50 rounded-lg border border-orange-200">
              <div className="text-center">
                <p className="text-sm text-gray-600 mb-1">Estimated Refund Amount</p>
                <p className="text-3xl font-bold text-orange-600">₹{result.refundAmount.toFixed(2)}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 bg-gray-50 rounded-lg">
                ...
                <p className="text-lg font-semibold text-gray-900">₹{result.originalFare.toFixed(2)}</p>
              </div>
              <div className="p-3 bg-red-50 rounded-lg">
                <p className="text-xs text-red-600">Deduction ({result.deductionPercentage}%)</p>
                <p className="text-lg font-semibold text-red-700">-₹{result.deductionAmount.toFixed(2)}</p>
              </div>
            </div>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between py-2 border-b border-gray-200">
                ...
                <span className="font-medium text-gray-900">{result.ticketType}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-gray-200">
                <span className="text-gray-600">Cancellation Time:</span>
                <span className="font-medium text-gray-900">{result.cancellationTime}</span>
              </div>
              <div className="flex justify-between py-2">
                <span className="text-gray-600">Processing Time:</span>
                <span className="font-medium text-gray-900">{result.processingTime}</span>
              </div>
            </div>
            ...
            {result.deductionPercentage < 100 && (
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                <p className="text-xs text-blue-800 flex items-start gap-2">
                  ...
                  <span>
                    {isHighValueRefund(result.refundAmount)
            ...
            {result.deductionPercentage === 100 && (
              <div className="p-3 bg-red-50 rounded-lg border border-red-200">
                <p className="text-xs text-red-800 flex items-start gap-2">
                  ...
                  <span>
                    {isHighValueRefund(result.originalFare)
                      ? `We understand this is disappointing. Unfortunately, no refund is available for this cancellation. ${result.ticketType === 'Tatkal' ? 'Tatkal tickets are non-refundable as per IRCTC terms for premium guaranteed-seat bookings.' : 'The cancellation is too close to departure time.'} However, if there are exceptional circumstances, you may consider filing a TDR (Ticket Deposit Receipt) for manual review. Would you like guidance on that?`
                      : `No refund available for this cancellation. ${result.ticketType === 'Tatkal' ? 'Tatkal tickets are non-refundable.' : 'Cancellation too close to departure time.'}`}
                  </span>
                </p>
              </div>
            )}
